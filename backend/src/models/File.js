...
class File {
  ...
  static async create({ user_id, originalname, gcs_path, folder_path, mimetype, size, is_folder = false }) {
    const result = await pool.query(
      `INSERT INTO user_files (user_id, originalname, gcs_path, folder_path, mimetype, size, is_folder, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, NOW()) RETURNING *`,
      [user_id, originalname, gcs_path, folder_path, mimetype, size, is_folder]
    );
    return result.rows[0];
  }
  ...
  static async update(id, updates) {
    ...
    const result = await pool.query(
      `UPDATE user_files SET ${setClause}, updated_at = NOW() WHERE id = $1 RETURNING *`,
      [id, ...values]
    );
    return result.rows[0];
  }
  ...
  static async moveToFolder(id, new_folder_path) {
    const result = await pool.query(
      'UPDATE user_files SET folder_path = $2, updated_at = NOW() WHERE id = $1 RETURNING *',
      [id, new_folder_path]
    );
    return result.rows[0];
  }
  ...
  static async rename(id, new_name) {
    const result = await pool.query(
      'UPDATE user_files SET originalname = $2, updated_at = NOW() WHERE id = $1 RETURNING *',
      [id, new_name]
    );
    return result.rows[0];
  }
  ...
  static async batchDelete(ids) {
    const result = await pool.query(
      'DELETE FROM user_files WHERE id = ANY($1) RETURNING *',
      [ids]
    );
    return result.rows;
  }
